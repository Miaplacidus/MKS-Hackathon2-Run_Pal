...
describe RunPal::FilterPostsByCircle do
  ...
  it 'filters posts by circle' do
    ...
    circle = RunPal.db.create_circle({name: "MakerSquare", admin_id: user.id, max_members: 30})
    post = RunPal.db.create_post({latitude: 30.25, longitude: -97.75, creator_id: user.id, max_runners: 10, time: Time.now, pace: 3, notes: "Fun!", min_amt: 12.50, age_pref: 3, gender_pref: 0, circle_id: circle.id})
