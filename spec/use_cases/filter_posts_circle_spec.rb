...
describe RunPal::FilterPostsByCircle do
  ...
  it 'filters posts by circle' do
    ...
    post = RunPal.db.create_post({location: [30.25, -97.75], creator_id: user.id, max_runners: 10, time: Time.now, pace: 3, notes: "Fun!", min_amt: 12.50, age_pref: 3, gender_pref: 0, committer_ids: [user.id], circle_id: circle.id})
